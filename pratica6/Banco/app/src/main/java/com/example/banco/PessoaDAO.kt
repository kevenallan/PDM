...
class PessoaDAO (context: Context) {
    ...
    fun getId(id: Int): Pessoa?{
        val colunas = arrayOf("id", "nome", "datahora")
        val where = "id = ?"
        val i = arrayOf(id.toString())
        val cursor = this.banco.readableDatabase.query("pessoas",colunas,where,i,null,null,null)
        cursor.moveToFirst()
        val pessoaId = cursor.getInt(cursor.getColumnIndex("id"))
        val pessoaNome = cursor.getString(cursor.getColumnIndex("nome"))
        val pessoaData = cursor.getLong(cursor.getColumnIndex("datahora"))
        return Pessoa(pessoaId,pessoaNome,pessoaData)
    }
